...
int main(int argc, char* argv[])
{
	...
	Scanner->SetLOSVariance(.1);
	Scanner->SetOrthogonalVariance(.1);
	
	Scanner->SetInput(reader->GetOutput());
	Scanner->Update();
	
	vtkPolyData* poly = Scanner->GetOutput();
	unsigned int NumPoints = poly->GetNumberOfPoints();
	if( ! ( (NumPoints == ThetaPoints * PhiPoints) || (NumPoints == ThetaPoints * PhiPoints + 1) )) //if rays are saved, there will be an extra point
	{
		//this test assumes that all of the rays that were cast hit the scene
		std::cout << "Failed!" << std::endl;
		std::cout << "NumPoints = " << NumPoints << " and ThetaPoints * PhiPoints = " << ThetaPoints* PhiPoints << std::endl;
		return EXIT_FAILURE;
	}
